...
function getListAccountsPayable(input) {
    ...
    var itemList = [];
    var file = null;
    var totalAmountPaidByPaymentId = {};
    ...
    try {
        ...
        while (rc == RC_SUCCESS) {
            ...
            if (totalAmountPaidByPaymentId[prepaymentId] === undefined) {
                totalAmountPaidByPaymentId[prepaymentId] = getTotalPayableAssetAmount(prepaymentId);
            }
            var totalAmountPaid = totalAmountPaidByPaymentId[prepaymentId];
            ...
            var item = {
                ...
                totalAmountPaid: totalAmountPaid,        // Tổng amount của payment entry PAYABLE/ASSET theo payment.id
                other_pending_amount: 0,   // Số tiền chờ duyệt ở các ĐNTT khác
...
function getTotalPayableAssetAmount(paymentId) {
    ...
    var query =
            ...
            'WHERE payment.id = "' + escapeSmQueryValue(paymentId) + '" ' +
            'AND entry.type = "PAYABLE" ' +
            'AND account.type = "ASSET"';

    try {
        entryFile = new SCFile("esdHTKTpaymentEntry", SCFILE_READONLY);
        var rc = entryFile.doSelect(query);

        while (rc == RC_SUCCESS) {
            totalAmount += getNumberField(entryFile, ["amount"]);
